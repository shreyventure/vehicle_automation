...
while True:
    ...
    if client.isApiControlEnabled() and car_state.speed < desired_speed - 1:
    ...
    if client.isApiControlEnabled():
        ...
        for response in responses:
            if response.pixels_as_float:
                ...
                image = cv2.resize(image, (320, 70), cv2.INTER_AREA)
                image = transformations(image)
                image = torch.Tensor(image)
                image = image.view(1, 3, 70, 320)
                image = Variable(image)
                main_steering_angle = model(image).view(-1).data.numpy()[0]
                main_steering_angle = float(main_steering_angle)
                print(f"st angle = {main_steering_angle}")

        if client.isApiControlEnabled() and (car_state.speed > desired_speed):
